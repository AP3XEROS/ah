...
class TradeUI extends PluginBase{
	...
	public function setEconomyProvider(EconomyProvider $provider){
        ...
        public function getBuyOrSellForm(Player $player) {
                ...
                $form->setTitle(RandomUtils::colorMessage("&a&k|&r&b&lAUCTION TRADE HOUSE&r&k&a|"));
                $content = "";
                $content .= RandomUtils::colorMessage("&l&8===========================\n");
                $content .= RandomUtils::colorMessage("&r&bHey there, {$username}!\n\n");
                $content .= RandomUtils::colorMessage("&r&bIf you want to buy items from the\n");
                $content .= RandomUtils::colorMessage("&r&bother players, click 'BUY'!\n\n");
                $content .= RandomUtils::colorMessage("&r&bIf you want to sell your own items\n");
                $content .= RandomUtils::colorMessage("&r&aclick 'SELL'!\n");
                $content .= RandomUtils::colorMessage("&l&8===========================\n");
                $form->setContent($content);
                $form->setButton(RandomUtils::colorMessage("&a&k|&r&b&lBUY&r&k&a|"));
                $form->setButton(RandomUtils::colorMessage("&a&k|&r&b&lSELL&r&k&a|"));
                $form->send($player);
        }
        ...
        public function getChooseItemForm(Player $player) {
                ...
                $form->setTitle(RandomUtils::colorMessage("&a&k|&r&b&lCHOOSE AN ITEM TO SELL&r&k&a|"));
                $this->savePlayerItemNames($player);
                $items = $player->getInventory()->getContents();
                foreach($items as $key => $item){
                        $form->setButton(RandomUtils::colorMessage("&l&k&a|&r&l&b{$item->getName()} &b(&8x{$item->getCount()}&b)&a&k|"),
        ...
        public function getSellForm(Player $player, Item $item) {
                ...
                $form->setTitle(RandomUtils::colorMessage("&a&k|&r&b&lSELL {$item->getName()}&r&k&a|"));
                $form->setInput(RandomUtils::colorMessage("&a&k|&r&b&lPRICE&r&k&a|"), "the price you're selling this item for.");
                $form->setSlider(RandomUtils::colorMessage("&b&lAMOUNT&r&a"), 1, $item->getCount(), 1, 1);
                $form->send($player);
        }
        ...
        public function getShopForm(Player $player, string $from = null) {
                ...
                if($from !== null){
                        ...
                        if(count($s) != 0){
                                ...
                                $form->setTitle(RandomUtils::colorMessage("&a&k|&r&b&l{$from}'s SHOP&r&k&a|"));
                                foreach($s as $r) {
                                        $this->cache[$username]["ids"][] = $r['id'];
                                        $form->setButton(
                                            RandomUtils::colorMessage(
                                                "&l&k&a|&r&l&b{$r['trade']} &b(&8x{$r['amount']}&b)&a&k|&r\n" .
                                                "&8Price - &6{$r['price']}&r"
                        ...
                        if($s !== null){
                                if(count($s) != 0){
                                        ...
                                        $page = $pg[1];
                                        $max = $pg[2];
                                        $form->setTitle(RandomUtils::colorMessage("&a&k|&r&b&lTRADE HOUSE SHOP&r&k&a|&r"));
                                        $form->setButton(RandomUtils::colorMessage("&l&k&a|&r&l&b← Last&a&k|"));
                                        $form->setContent(RandomUtils::colorMessage(
                                            "            &l&bTotal of &7$items &bitems\n" .
                                            "          &l&a- &r&l&bPage &7$page &bof &7$max&a -"
                                        ));
                                        $i = 1;
                                        ...
                                        $form->setButton(RandomUtils::colorMessage("&l&k&a|&r&l&bNext →&a&k|"));
                                        $form->send($player);
                                }else{
        ...
        public function getMyOffersUI(Player $player){
                ...
                if(count($s) != 0){
                        ...
                        $form->setTitle(RandomUtils::colorMessage("&a&k|&r&b&lYOUR OFFERS&r&k&a|"));
                        $this->cache[$username]["ids"] = [];
                        foreach($s as $r){
                                $this->cache[$username]["ids"][] = $r['id'];
                                $form->setButton(RandomUtils::colorMessage("&l&k&a|&r&l&b{$r['trade']} &b(&8x{$r['amount']}&b)&a&k|"), "http://permat.comli.com/items/{$r['itemId']}-{$r['itemMeta']}.png");
                        }
                        $form->send($player);
                }else{
        ...
        public function getConfirmPurchaseForm(Player $player, int $id) {
                ...
                if(count($dat) > 0){
                        ...
                        $form->setTitle(RandomUtils::colorMessage("&a&k|&r&b&lDO YOU WANT TO BUY THIS ITEM?&r&k&a|"));
                        $content = "";
                        $content .= RandomUtils::colorMessage("&l&a===========================&r\n");
                        $content .= RandomUtils::colorMessage("&bSeller: &7" . $dat['username'] . "\n");
                        $content .= RandomUtils::colorMessage("&bItem Name: &7" . $dat['trade'] . " &r&b(" . Item::get($dat['itemId'])->getName() . ")\n");
                        $content .= RandomUtils::colorMessage("&bPrice: &7" . $dat['price'] . "\n");
                        $content .= RandomUtils::colorMessage("&bAmount: &7" . $dat['amount'] . "\n");
                        $content .= RandomUtils::colorMessage("&bEnchantments: &7" . $this->formatEnchantmentIdsAsName($dat['enchants']) . "\n");
                        $content .= RandomUtils::colorMessage("&bMarket ID: &7" . $dat['id'] . "\n");
                        $content .= RandomUtils::colorMessage("&l&a===========================\n");
                        $form->setContent($content);
        ...
        public function getConfirmDeleteForm(Player $player, int $id) {
                ...
                if(count($dat) > 0){
                        ...
                        $form->setTitle(RandomUtils::colorMessage("&a&k|&r&b&lDO YOU WANT TO REMOVE THIS ITEM?&r&k&a|"));
                        $content = "";
                        $content .= RandomUtils::colorMessage("&l&a===========================&r\n");
